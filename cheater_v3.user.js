...
// @include		http://*.camorraworld.nl/*
// @include		http://*.camorraworld.org/*
...
// Settings migration
if (!getSetting('newSettings', false))
{
	var userId = getUID();
	for each (var val in GM_listValues())
	{
		var index = val.indexOf('_' + userId);
		if(index !== -1)
		{
			var settingName = val.slice(0, index);
			saveSetting(settingName, GM_getValue(val));
		}
	}
	saveSetting('newSettings', true);
}
		...
		{ 
			...
			$(rows).each(function ()
			{
				if (!newMessage)
				{
					...
					if ((status.html().indexOf("/style/1/images/icons/eb.png") !== -1) || (status.html().indexOf("/style/1/images/icons/en.png") !== -1))
					{
						newMessage = true;
						return false;
					}
...
if (getSetting('firstRun', true) && getSetting('username') === null && unsafeWindow.USERNAME !== undefined)
{
	manualPause();
...
function openIntroStep3(dialog, dialogContent)
{
	...
	$(dialogContent).html("Dit waren alle basisinstellingen. De rest van de instellingen zijn crimes. In de volgende stappen lopen we ze allemaal even door. Als je <b>̬cht alles</b> al weet kun je de wizard sluiten en anders druk je nu op \"Volgende\".");
	$(dialog).dialog( {
		buttons:
		{
			"Afsluiten": function ()
...
{
	$(dialogContent).html("Voor het jatten van auto's zijn ook twee keuzes, GTA en GTA Ultra. Heel simpel, kies ̩̩n van de twee uit de keuzelijst en alles komt goed.");
	var cars = $('#auto');
...
function getSetting(settingName, defaultValue){ // returnvalue: correct setting
    var username = unsafeWindow.USERNAME;
	if (defaultValue === null || defaultValue === undefined)
    {
        value = GM_getValue(settingName + "_" + username);
		if (value === undefined)
		{
			return null;
		}
    ...
    return GM_getValue(settingName + "_" + username, defaultValue);
}
function saveSetting(settingName, value)
{
	var username = unsafeWindow.USERNAME;
	GM_setValue(settingName + "_" + username, value);
}
function removeSetting(settingName)
{
	var username = unsafeWindow.USERNAME;
	GM_deleteValue(settingName + "_" + username);
}
